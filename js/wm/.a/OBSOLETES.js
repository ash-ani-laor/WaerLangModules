...
class _SVG {
    ...
    /* Some obsolete functions
      CreateProperties () {
        ...
        this.Mok1 = 30;//120;
        this.DotSize = .01;
        this.FontParams["font-size"] = 4;//20;
        this.HolyCoefficient = 1;//17; //stupidity used in positioning of Q subscription
        this.DotColor = "#eaeaea";
        this.TextAngle = 0;
        this.TextOffsetX = 0; //offset of text from dot or center of some line/arc
        this.TextOffsetY = 0;//this.Mok1/3; //offset of text from dot or center of some line/arc
        this.hslSVal = "50%";
        this.hslLVal = "50%";
        ...
        this.Object.Neighbours = {};
        //[0|1] for focuses, for each focus an object with dots' array, subdivided into zones
        this.Object.Circles = [{ Dots: _.range(this.Xer1).map(function () { return []; }) },
          { Dots: _.range(this.Xer1).map(function () { return []; }) }];
        this.Object.Q = {};
        this.Object.QLS = [];
        this.Object.TotalLetters = 0;
        this.Object.I1Z1TL = 0;
        this.Object.QDots = 0;
        this.Object.I1Z1QD = 0;
        this.Object.FE = [];//_.range(this.Xer1-1).map(function () {return 0;}); //First Ellipse, where we count only upper (k=1) zone
        this.Object.Base = [];
        this.Object.CardAx = [];
        this.Object.Dots = _.range(this.Xer1 * 1.5).map(function () { return _.range(this.Xer1 * 1.5).map(function () { return [0, 0, 0]; }); });
        this.Object.Values = _.range(this.Xer1 * 1.5).map(function () { return _.range(this.Xer1 * 1.5).map(function () { return [0, 0, 0]; }); });
        this.DotValueToShow = {
          Values: false,
          Position: false,
      ...
      SetParams () {
        ...
        this.DotValueToShow.FullGematrias = true;
        this.DotValueToShow.Values = true;
        //this.DotValueToShow.Position = true;
        //this.DotValueToShow.DotType = true;
        //this.DotValueToShow.Neighbours = true;
        //this.QuadratValueToShow.ColorRVALOnly = true;
        //this.QuadratValueToShow.ColorLVALOnly = true;
        //this.QuadratValueToShow.PValues = true;
        //this.QuadratValueToShow.Values = true;
        //this.QuadratValueToShow.ValGems = true;
        //this.QuadratValueToShow.QNum = true;
        this.QuadratValueToShow.GemValsSep = true;
        //this.QuadratValueToShow.TotalGematria = true;
        this.QuadratValueToShow.OrdinalSum= true;
        this.FontParams["font-family"] = "code2000";
        //"WaerIasen";
        //this.FontParams["font-weight"] = "bold";
        this.MaxVal = Math.floor(this.Xer1 * 1.5);
      }
    */
  
      _circle (cx, cy, r, name) {
        var p = "M " + cx + "," + cy + " " +
          "m -" + r + ",0 " +
          "a " + r + "," + r + " 0 1 0 " + (r * 2) + ",0" +
          "a " + r + "," + r + " 0 1 0 -" + (r * 2) + ",0" +
      ...
      _dot (cx, cy, name, value) {
        var d = WD.circle(cx, cy, this.DotSize).attr(this.Params);
        $(d.node).attr('id', name);
        if (typeof value !== "undefined") {
          var t = this._text(value, cx - this.TextOffsetX, cy - this.TextOffsetY, "t_" + value);
          t.attr(this.FontParams);
          $(d.node).data("Txt", t);
        }
        d.attr({ fill: this.DotColor });
        return d;
      }
      _subscribe_dot (dot) {
        var cx = dot.DotCoords.x - this.TextOffsetX,
          cy = dot.DotCoords.y - this.TextOffsetY,
          name = "DT_" + dot.Position[0].toString(36) + "_" + dot.Position[1] + "_" + dot.Position[2].toString(36),
          value = this._curValue(dot), t;
        if (this.Object.Values[dot.Position[0]][dot.Position[1]][dot.Position[2]] !== 0) {
          this.Object.Values[dot.Position[0]][dot.Position[1]][dot.Position[2]].attr('text', value);
        } else {
          t = this._text(value, cx, cy, name).attr(this.FontParams);
          this.Object.Values[dot.Position[0]][dot.Position[1]][dot.Position[2]] = t;
        }
      }
      _curValue (dot) {
        var f = this.DotValueToShow, value = "";
        if (f.Values) {
      ...
      _line_d (sd, ed, name, value) {
        var path = "M" + sd.DotCoords.x + "," + sd.DotCoords.y,
          l = " L" + ed.DotCoords.x + "," + ed.DotCoords.y;
        var line = WD.path(path + l).attr(this.Params);
        if (typeof value !== 'undefined' && this.DotValueToShow.SubscribeLines) {
          var point = line.getPointAtLength(line.getTotalLength() / (1.49)); //get center of line;
          point.x -= this.TextOffsetX / 2;
          point.y -= this.TextOffsetY / 2;
          var text = this._text(value, point.x, point.y, "LV_" + name).attr({ fill: "blue" });
          $(line.node).data('Txt', text);
          $(line.node).data('Val', value);
        }
        $(line.node).attr('id', name);
        return line;
        this.Object.Lines["P_" + name] = line; //we name'em by pairs' names
        this.Object.Paths.Lines["P_" + name] = l;
      }
      _path (path, name, value, ccs) { //path to draw and center coordinates
        var l = WD.path(path).attr(this.Params).toBack();
        $(l.node).attr("id", name);
        if (typeof value !== "undefined") {
          if (typeof ccs !== "undefined") {
            var t = this._text(value, ccs.x, ccs.y, "PL_".name).attr({ fill: "black" });
            $(l.node).data("Txt", t);
      ...
      _line_c (sx, sy, ex, ey, name, value) {
        ...
        if (typeof value !== 'undefined') {
          ...
          point.x -= this.TextOffsetX / 2;
          point.y -= this.TextOffsetY / 2;
          var text = this._text(value, point.x, point.y, "Line_" + name + "_Value");
          $(line.node).data('Txt', text);
          $(line.node).data('Val', value);
        }
        line.attr(this.Params);
        $(line.node).attr('id', name);
        return line;
      }
      _text (text, sx, sy, name) {
        var txt = WD.text(sx, sy, text).rotate(this.TextAngle).attr(this.FontParams);
        $(txt.node).attr('id', name);
        return txt;
      }
      _change_text () {
        _.each(this.Object.Dots, function (element, index, list) {
          _.each(element, function (elem, ind, li) {
            _.each(elem, function (el, i, l) {
              if (el !== 0) {
                this._subscribe_dot(el);
              }
            });
          });
        });
      ...
      _redraw_lines () {
        if (typeof this.Object.Lines === "undefined") {
          return;
        }
        for (var line in this.Object.Lines) {
          if (!this.DotValueToShow.Neighbours) {
            if (typeof _.find(_.keys(this.Object.Neighbours), function (name) {
              return name === line;
            }) !== "undefined") {
              this.Object.Lines[line].hide();
            }
          } else {
            if (typeof _.find(_.keys(this.Object.Neighbours), function (name) {
              return name === line;
            }) !== "undefined") {
              this.Object.Lines[line].show();
            }
          }
        }
      }
      _arc (focus, sd, ed, name, value) { //focus is 1 or 2
        var d = this._dim(),
          //rey = Math.abs(d.c.y - sd.DotCoords.y)*3,
          rex = d.r / 2 + this.Mok1 / 2;
        ...
        var ee = WD.path(pe).attr(this.Params);
        if (typeof value !== 'undefined') {
          var point = ee.getPointAtLength(ee.getTotalLength() / 2); //get center of line;
          point.x -= this.TextOffsetX / 2;
          point.y -= this.TextOffsetY / 2;
          var text = this._text(value, point.x, point.y, "AVal_" + name).attr(this.FontParams);

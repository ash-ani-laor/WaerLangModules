...
class WSolidSimplex extends WGeom {
  //https://habr.com/ru/post/168421/ — жизнь по-лобачевски
  //https://habr.com/ru/post/247449/ — правильные многогранники
  //https://megalektsii.ru/s56751t3.html - координаты вершин многогранника
  //https://en.wikipedia.org/wiki/N-sphere#Spherical_coordinates
  simplex = {
      height: () => {this.side * Math.sqrt((this.dim + 1) / 2 * this.dim)},
      r_circumscribed: () => {this.side * Math.sqrt(this.dim / (2 * (this.dim + 1)))}
  }
  constructor (dim = 3, side = 1) {
      super()
      this.dim = dim
      this.side = side
      window.D = (v, sz=this.DotSize*2, clr="goldenrod") =>{
          this.dot(v, sz, clr)
          this.line(this.O, v)
      }
      window.I = (s, e, clr=this.WG.getRandomColor()) => {
          this.backupColor()
          this.Color = clr
          return this.line(s, e)
      }
      this.WG.infiniteGrid()
  }
  s2D () {
      //center at this.O, up at z (change to minus)
      //in spherical:
  }
  c_S2C_3D (sc) { //spherical to cartesian
      let ret =  this.V([0,0,0]).setFromSphericalCoords(sc[0], sc[1], sc[2])
      return ret
  }
  c_C2S_3D(v3) { //cartesian to spherical
      let
          r = Math.sqrt ( v3.x**2 + v3.y**2 + v3.z**2 ),
          f = Math.acos ( v3.z / r ),
          t = Math.atan2 ( v3.y, v3.x )
      return [r, f, t]
  }
}
///////////////////////////////////////////////////////////////

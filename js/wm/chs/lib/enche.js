...
var Judge = {
   ...
   * стр. 124
   * FIXME: Problems seen:
   * — Ферзь, попадающий на Трон с обеими фигурами, не съедает никого
   * — Подёргал неактивным ферзём на этой клетке, чтобы вытащить активного Короля, не удалось
   * — Водная Ладья не смогла съесть фигуру противника через три верт. свободные клетки -
   *      вернулась как неправильный ход, также не смогла горизонтально съесть в соседней клетке фигуру,
   *      зато ест по диагонали. Пойти по диагонали не может. Нормально идёт на 1 шаг вниз, затем способна съесть через 2 клетки
   *      с другой стороны через всё поле съела, выбрав слона с Трона, по горизонтали тоже удалось съесть фигуру
   * — Пешки умудряются ходить в любую недиагональную сторону, и даже назад.
   *    Огненная пешка, пойдя влево, не может диагонально налево-вверх съесть
   * —
   */
  ...
  cell_get_data: function (id) {
    ...
    if (data[0] === 'row') {
      ...
      return {
        cQua: data[1],
        cRow: parseInt(data[2], 10),
        cCel: parseInt(data[3], 10),
...
var enochian_colorize = function () {
  ...
  $('.cell').each(function (i, c) {
    ...
    $(c).css({"border-right": subquad, 'border-left': table, 'border-bottom': lower_color, 'border-top': upper_color, opacity: .5});
    ////////////////////////////////////////////////
...
var put_enochian_figures = function () {
  ...
  $('.cell').each(function (i, o) { //row_Y_Z_N, c_Z_N_I //where Y - table, Z - quadrant, N - row, I - cel
    ...
    for (a = 0; a < figs.length; a++) {
      ...
      //let us try make all figures children of some special div
      $("#figpar").append("<div class='fig' id='fig_" + figs[a] + "_" + cq.cQua + "_" + i + "'></div>") //figure id: fig_F_Y_O

      $(o) //here we create a figure and set data to its parent cell.
      //FIXME:
        .addClass('occupied')
      ...
      $(name)
        ...
        .data('moveNo', 0)
        .offset({
          top: $(o).offset().top - Rules.CellSize*.5,
          left: $(o).offset().left + Rules.CellSize*.5
        })
        /*.qtip({
          show: {
            event: 'dblclick',
...
var recommendationsOnPtaxPosition = function() {
...
Ptax(); recommendationsOnPtaxPosition();
